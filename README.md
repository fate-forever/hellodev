...
HelloDev is a local, deterministic orchestration and governance CLI for AI-assisted development. It keeps repository workflow (Trellis), optional long-term memory (Nocturne), and the actual Agent host as separate authorities. Core provides bounded context, explicit approvals, receipts/Sagas, crash recovery, typed Host integration, policy canaries, checkpoints, and a read-only Control Center. It does not vendor upstream systems, execute models, auto-write memory, or treat host-reported usage as provider-attested evidence.
...
PyPI publishing is intentionally separate from this source release. Do not assume `pip install hellodev-core` is available from the public index until the project publishes and verifies it explicitly.
...
所有 HelloDev 运行状态位于选定项目的 `.hellodev/`。0.8 至 0.12.0 项目可以直接由 0.12.1 打开；transaction、pending HostEnvelope 和 checkpoint store 都按需创建，不复制 Trellis/Nocturne 数据。
...
```powershell
python -m pip wheel . --no-deps --no-cache-dir --wheel-dir dist
Get-FileHash .\dist\hellodev_core-0.12.1-py3-none-any.whl -Algorithm SHA256
```
...
0.12.1 在 0.12.0 基线上完成可靠性和开源发布收口。最终测试数、wheel/hash、Demo 和隔离 smoke 证据见 [RELEASE.md](docs/RELEASE.md) 及独立发布目录；既有 release 保持不可变。

以下 **0.11.0 历史发布证据保持不变**：
