...
class V121OssTests(unittest.TestCase):
    def test_version_typing_ci_and_public_docs_are_aligned(self) -> None:
        ...
        self.assertEqual(__version__, "0.19.6")
        self.assertIn('version = "0.19.6"', pyproject)
        ...
        self.assertIn("HelloDev Core 0.19.6", readme)
        self.assertIn("HelloDev 0.19.6", quick_start)
        self.assertIn("Windows x86_64", quick_start)
        self.assertIn("components verify", quick_start)
        self.assertIn("本文不宣称 HelloDev 0.19.6 已发布到 PyPI", readme)
        ...
        self.assertIn("## 0.19.0：自适应日常编排", readme)
        self.assertIn("--query \"修复登录超时\"", readme)
        self.assertIn("Context Plane：不用另装 FastCtx", quick_start)
        self.assertIn("optional accelerator", readme)
        self.assertIn("open -> next -> do", quick_start)
        self.assertIn("Control Center 2.6", quick_start)
        ...
        self.assertLess(
            quick_start.index("复制给 Codex / Cursor / Antigravity Agent"),
            quick_start.index("手工安装参考"),
        )
        for path in (
