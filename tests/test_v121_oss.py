...
class V121OssTests(unittest.TestCase):
    def test_version_typing_ci_and_public_docs_are_aligned(self) -> None:
        ...
        self.assertEqual(__version__, "0.14.3")
        self.assertIn('version = "0.14.3"', pyproject)
        ...
        self.assertIn("HelloDev Core 0.14.3", readme)
        self.assertIn("HelloDev 0.14.3", quick_start)
        self.assertIn("Windows x86_64", quick_start)
        self.assertIn("components verify", quick_start)
        self.assertIn("本文不宣称 HelloDev 0.14.3 已发布到 PyPI", readme)
