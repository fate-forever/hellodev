...
```powershell
pipx install C:\path\to\hellodev_core-0.11.0-py3-none-any.whl
hellodev --version
```
...
python -m pip install C:\path\to\hellodev_core-0.11.0-py3-none-any.whl
hellodev --version
```
...
```text
请在当前仓库使用已安装的 HelloDev 0.11.0 完成我接下来描述的任务。
...
9. 持续推进到任务完成或出现真实阻塞。结束时汇报改动、测试/门禁证据、剩余风险和下一条建议；token/subagent 用量只有宿主提供可信回执时才报告精确值，否则写 unavailable。
...
Codex 和 Cursor 的差别只在宿主如何展示终端调用与权限弹窗；HelloDev 命令、项目状态和授权规则相同。不要为了省一次确认而开启外部写入的自动执行。
...
打开 `dashboard start` 返回的 `127.0.0.1` 地址。页面可以查看 lifecycle、能力、usage、optimization、host/policy/drift 摘要并复制建议命令。
...
| token 显示 unavailable | 当前宿主没有提供可信用量回执；HelloDev 不估算精确值 |
...
- 0.11.0 host/policy/drift 示例：[EVOLUTION_DEMO.md](EVOLUTION_DEMO.md)
